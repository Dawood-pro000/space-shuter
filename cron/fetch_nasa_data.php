...
$agent1Instruction = "You are Key #1: The Ingestion Architect for Project Space Shutter. Your sole job is to parse and synthesize incoming raw text parameters from NASA data. Consolidate discovery, key parameters, and image URLs into a clean, compact, structured JSON object. Maintain strict data integrity. Deliver raw, accurate, filtered facts.";

$agent1Prompt = "Parse the following NASA JSON response. Return ONLY a JSON object with exact keys: 'abstract' (the main explanation/facts), 'image_url', 'title'.\n\nData:\n" . $nasaResponse;

$ingestedData = callGemini($agent1Prompt, $geminiKey1, $agent1Instruction);

if (!$ingestedData || !isset($ingestedData['abstract'])) {
...
$agent2Prompt = "Take this factual JSON packet and return ONLY a JSON object with exact keys: 'headline' (exciting non-clickbait title), 'content' (rewrite the text into clean HTML components like <p> and <strong> suitable for our frontend, DO NOT put the title inside the HTML).\n\nPacket:\n" . json_encode($ingestedData);

$creativeData = callGemini($agent2Prompt, $geminiKey2, $agent2Instruction);

if (!$creativeData || !isset($creativeData['content'])) {
...
$supabaseEndpoint = "{$supabaseUrl}/rest/v1/posts";
...
if ($supabaseHttpCode >= 200 && $supabaseHttpCode < 300) {
    echo "SUCCESS: Post '{$title}' generated by AI pipeline and inserted into Supabase!\n";
} else {
